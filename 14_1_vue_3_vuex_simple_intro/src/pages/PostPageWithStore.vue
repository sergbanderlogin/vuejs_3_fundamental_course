<template>
  <div>
    <h1>{{$store.state.isAuth ? 'Корисувач авторизований' : 'Щоб користуватися сервісом, потрібно авторизуватися'}}</h1>
    <h1>Likes: {{ $store.getters.doubleLikes }}</h1>
    <div>
      <my-button @click="$store.commit('incrementLikes')">Лаайк</my-button>
